...
--		5.1	Database performance - Using * in table with lots of columns might affect database performance
--		5.2 Application performance - Retrieving unnecessary data from the increases the traffic
...
-- 3.	Get movie_name, movie_lang from movie table
SELECT
	movie_name, movie_lang
FROM movies;

-- 4.	Using explicit columns is a good practice and produce less load on the server end
...
-- 4.	Make alias for movie_name as 'Movie Name' and movie_lang as 'Language' of movies table
SELECT
...
-- Column aliases can used for derived columns
...
-- Yes. Using Expression
-- We will us || as the concatenating operator
SELECT
	first_name || ' ' || last_name
FROM actors;
...
-- 4.	Can I just expressions to get output without using a table column?
-- Yes. You can
...
-- To sort the rows of the results set, we use the ORDER BY clause in the SELECT statement.
...
FROM tablename
ORDER BY
	sort_expression [ASC | DESC] [NULL FIRST | NULL LAST]
*/

-- 1. To demostrate, lets create a table called demo_sorting
...
-- Selecting distinct calues
-- ############################################

-- SELECT DISTINCT columnname FROM tablename

-- 1.	Lets get all records from movie table
SELECT *
FROM movies;
